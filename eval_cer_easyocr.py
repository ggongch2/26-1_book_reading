"""
OCR CER 평가 - EasyOCR / bbox 크롭 방식
=========================================
사용법:
    # 폴더
    python eval_cer_easyocr.py --input ./validation

    # 분리 zip (AIHub 구조)
    python eval_cer_easyocr.py --label_zip "[라벨]train.zip" --img_zips "[원천]train1.zip" ...

    python eval_cer_easyocr.py --label_zip ... --img_zips ... --max 200
"""
...
def ocr_crop(reader: easyocr.Reader, crop: np.ndarray) -> str:
...
def eval_loop(reader, pairs_iter, total, save_path):
    all_results = []
    ...
    for i, (name, items, img_np) in enumerate(pairs_iter):
        ...
        for item in items:
            ...
            pred = ocr_crop(reader, crop) if crop is not None else ""
...
if __name__ == "__main__":
    ...
    if args.label_zip:
        ...
        total = min(len(pairs), args.max) if args.max else len(pairs)
        eval_loop(reader, iter_from_dir(pairs, args.max), total, args.out)
    else:
        parser.print_help()
